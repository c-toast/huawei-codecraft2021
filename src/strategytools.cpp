...
int CalFitness(ServerInfo &serverInfo, VMInfo &vmInfo, double &fitnessReceiver) {
    ...
    fitnessReceiver=(double(vmCpuNum)/serverCpuNum+double(vmMemSize)/serverMemSize)*(hardCost+averageDay*energyCost);
    return 0;
    */
}
...
double CalCost(ServerInfo *serverInfo){
    ...
    int hardCost=serverInfo->hardwareCost;
    int energyCost=serverInfo->energyCost;
    double cost=double(serverCpuNum+serverMemSize)/(hardCost+energyCost);
    return cost;
}