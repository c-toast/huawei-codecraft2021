...
//    receiver=*serverObj;
    //fake server do not copy the map structure
    receiver.id = serverObj->id;
    receiver.nodes[0] = serverObj->nodes[0];
    receiver.nodes[1] = serverObj->nodes[1];
    receiver.info = serverObj->info;
...
int CloudOperator::deployVMObjInFakeServerObj(ServerObj *serverObj, VMObj *vmObj, int nodeIndex) {
    ...
    serverObj->allocResForDeploy(nodeIndex, vmObj);
    return 0;
}

int CloudOperator::delVMObjInFakeServerObj(ServerObj *serverObj, int nodeIndex, VMObj *vmObj) {
    int serverID = serverObj->id;
    if (serverID == -1 || globalCloud->serverObjList[serverID] == serverObj) {
        LOGE("CloudOperator::deployVMObjInFakeServerObj: deploy on a new or real serverObj!");
        return -1;
    }
    serverObj->freeResForDel(nodeIndex, vmObj);
...
bool MigrateDependencyTree::isAncestor(VMObj *parent, VMObj *child) {
    std::map<VMObj *, bool> haveBeenSearch;
    return isAncestorRecursive(parent,child,haveBeenSearch,0);
}

bool MigrateDependencyTree::isAncestorRecursive(VMObj *parent, VMObj *child, std::map<VMObj *, bool> &haveBeenSearch,
                                                int layerNum) {
    //    if(layerNum==10){
//        LOGE("break point");
//    }
    ...
    //prune
    if(haveBeenSearch.find(child)!=haveBeenSearch.end()){
        return false;
    }else{
        haveBeenSearch[child]=true;
    }
    ...
    for (auto it:childNode->parents) {
        ...
        if (isAncestorRecursive(parent, it->vmObj,haveBeenSearch,layerNum+1)) {
            return true;
        }
    }
    return false;
